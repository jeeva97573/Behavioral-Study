...
import numpy as np  # whole numpy lib 
from numpy import (sin, cos, tan, log, log10, pi, average, sqrt, std, deg2rad, rad2deg, linspace, asarray)
from numpy.random import random, randint, normal, shuffle
import os  # for handy system and path functions
import sys  # to get file system encoding

from psychopy.hardware import keyboard

# To ensure that relative paths start from the same directory as this script
...
if dlg.OK == False:
    core.quit()  # if the participant pressed cancel
expInfo['date'] = data.getDateStr()  # to add a simple timestamp
...
# to save a log file for detail verbose info
logFile = logging.LogFile(filename+'.log', level=logging.DEBUG)
logging.console.setLevel(logging.WARNING)  # to output to the screen

endExpNow = False  # 'escape' => quit the exp
...
# store frame rate of monitor to measure it
...
while continueRoutine:
    ...
    if intro.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # to keep track of start time/frame for later
    ...
    if intro.status == STARTED:
        # to determine if its time to stop
        if tThisFlipGlobal > intro.tStartRefresh + 2.0-frameTolerance:
            # to keep track of stop time/frame for later
    ...
    if instruction_resp.status == STARTED and not waitOnFlip:
        ...
        if len(_instruction_resp_allKeys):
            instruction_resp.keys = _instruction_resp_allKeys[-1].name  # when the last key is pressed
    ...
    # to check if all components have finished
    if not continueRoutine:  # if a component has requested a forced-end of Routine
        break
...
if instruction_resp.keys != None:  # when we get a response
...
# set up handler to look after randomisation of conditions mentioned in the file 'conditions.xlsx'
...
# to initialise stimuli with some values
if thisTrial != None:
...
for thisTrial in trials:
    ...
    while continueRoutine:
        ...
        if resp.status == NOT_STARTED and tThisFlip >= 0.7-frameTolerance:
            ...
            win.callOnFlip(resp.clearEvents, eventType='keyboard')  # to clear events on next screen flip
        if resp.status == STARTED and not waitOnFlip:
            theseKeys = resp.getKeys(keyList=['left', 'right'], waitRelease=False)
            _resp_allKeys.extend(theseKeys)
            if len(_resp_allKeys):
                resp.keys = _resp_allKeys[-1].name  # when the last key is pressed
                resp.rt = _resp_allKeys[-1].rt
                # a response ends the routine
                continueRoutine = False
        
        # to check for quit (Esc key)
        if endExpNow or defaultKeyboard.getKeys(keyList=["escape"]):
            core.quit()
    ...
    if resp.keys != None:  # if a response was made
...
# to get names of stimulus parameters
...
# To ending Routine "end"
for thisComponent in endComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
thisExp.addData('ending.started', ending.tStartRefresh)
thisExp.addData('ending.stopped', ending.tStopRefresh)
# the Routine "end" was not non-slip safe, so it is necessary to reset the non-slip timer
routineTimer.reset()

# to flip one final time so any remaining win.callOnFlip() and win.timeOnFlip() tasks get executed before quitting
win.flip()
...
logging.flush()
# to make sure everything is closed down
thisExp.abort()  # to save data files on exit
win.close()
core.quit()
